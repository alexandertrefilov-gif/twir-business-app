...
export function OrderStatusActions({
  ...
  return (
    <div className="space-y-2">
      ...
      {/* COMPLETED */}
      {status === 'COMPLETED' && (
        <a href={`/invoices/new?order=${orderId}`}
          className="w-full flex items-center gap-2 h-9 px-4 rounded-md bg-blue-700 hover:bg-blue-800 text-white text-sm font-500 transition-colors">
          <InvoiceIcon /> Rechnung erstellen
        </a>
      )}

      {(status === 'INVOICED' || status === 'CANCELLED') && (
        <p className="text-xs text-muted-foreground text-center py-2">Keine weiteren Aktionen</p>
      )}
    </div>
  )
}
...
function InvoiceIcon() { return <svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg> }
