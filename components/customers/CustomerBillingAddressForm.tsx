...
import type { BillingAddressActionState } from '@/app/(dashboard)/customers/[id]/billing-addresses/actions'

type Defaults = Partial<Record<'label'|'companyName'|'additional'|'street'|'houseNumber'|'postalCode'|'city'|'country'|'contactName'|'email', string>> & {
  isActive?: boolean
  isDefault?: boolean
}

export function CustomerBillingAddressForm({ action, defaults = {}, mode, sharedWith = [] }: {
  action: (state: BillingAddressActionState, data: FormData) => Promise<BillingAddressActionState>
  defaults?: Defaults
  mode: 'create' | 'edit'
  ...
  return <form action={formAction} className="space-y-4">
    ...
    <div className="form-section">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <label className="flex items-center gap-2 text-sm"><input name="isActive" type="checkbox" checked={active} onChange={event => setActive(event.target.checked)} /> Für neue Rechnungen auswählbar</label>
        <label className="flex items-center gap-2 text-sm"><input name="isDefault" type="checkbox" defaultChecked={defaults.isDefault} disabled={!active} /> Standard-Rechnungsadresse</label>
      </div>
    </div>
