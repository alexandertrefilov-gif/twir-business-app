...
export function ExternalConfirmationDialog({ entityType, entityId, hasCustomerPurchaseOrder = false, confirmationType, confirmedAt, confirmationNote }: {
  entityType: 'order'
  entityId: string
  ...
  return <Dialog.Root open={open} onOpenChange={next => !pending && setOpen(next)}>
    <Dialog.Trigger asChild><button type="button" className="min-h-9 w-full rounded-md bg-blue-700 px-3 py-2 text-sm font-500 text-white hover:bg-blue-800">{confirmationType ? 'Bestätigung bearbeiten' : 'Auftrag bestätigen'}</button></Dialog.Trigger>
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[90vh] w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl border border-stone-200 bg-white p-6 shadow-2xl" aria-describedby={`confirmation-description-${entityId}`}>
        <Dialog.Title className="text-base font-600">Auftrag bestätigen</Dialog.Title>
        ...
        {(type === 'SIGNED_DOCUMENT' || Boolean(confirmationType)) && <div onDragOver={event => event.preventDefault()} onDrop={drop} className="mt-4 rounded-lg border border-dashed border-stone-300 bg-stone-50 p-5 text-center">
          <p className="text-sm font-500">{type === 'SIGNED_DOCUMENT' ? 'Bestätigung hier hineinziehen' : 'Zusätzliches Bestätigungsdokument (optional)'}</p>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button type="button" disabled={pending || (type === 'SIGNED_DOCUMENT' && !file)} onClick={submit} className="h-9 rounded-md bg-blue-700 px-4 text-sm font-500 text-white disabled:opacity-50">{pending ? 'Wird gespeichert…' : 'Auftrag bestätigen'}</button>
        </div>
